...
test('끝말잇기는 게임 시작 시 참가 순서를 섞어 방장 선턴을 고정하지 않는다', async () => {
  resetWordChainGamesForTest();
  ...
  const common = {
    guildId: 'guild-1',
    channelId: 'channel-random-order',
    channel
  };
  ...
  const lobbyComponents = replies[0].components[0].toJSON().components;
  const joinButton = lobbyComponents.find((component) => component.label === '참가');
  const startButton = lobbyComponents.find((component) => component.label === '방장 시작');

  for (const user of [
    { id: 'user-2', username: '둘째', bot: false },
    { id: 'user-3', username: '셋째', bot: false }
  ]) {
    await handleWordChainCommand({
      ...common,
      user,
  ...
  assert.match(sent[0], /순서\(랜덤\):/);
  assert.match(sent[0], /1\. <@user-2>/);
  assert.match(sent[1], /<@user-2> 차례/);
  assert.doesNotMatch(sent[1], /<@host-1> 차례/);
