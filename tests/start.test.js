...
test('시작하기 새로고침 버튼은 가이드를 다시 계산해 업데이트한다', async () => {
  ...
  const handled = await handleStartCommand(interaction, createServices({
    ...
    miningStatus: { stats: { totalMines: 1 } },
  ...
  assert.match(interaction.updates[0].content, /진행도 8\/8/);
...
function createServices({
  ...
  miningStatus = { stats: { totalMines: 0 } },
  ...
  return {
    ...
    mining: {
      async getProfile() {
        if (failing.has('mining')) throw new Error('mining unavailable');
        return miningStatus;
      }
    },
    stocks: {
