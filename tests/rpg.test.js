...
test('RPG 카드 본문은 긴 진행 설명도 생략 안내 문구 없이 안전 길이로 제한한다', () => {
  const content = [
    '🧙 **RPG 긴 화면**',
  ...
  assert.match(payload.embeds[0].data.description, /아주 긴 설명/);
  assert.doesNotMatch(payload.embeds[0].data.description, /생략/);
});

test('RPG 카드 본문 fallback은 튜토리얼형 입력에도 생략 안내 문구를 노출하지 않는다', () => {
  const content = [
    '🎓 **RPG 초보자 여정**',
    '진행도: **0/6 보상 수령**',
    '다음 목표: **캐릭터 생성**',
    ...Array.from({ length: 6 }, (_, index) => [
      `- **${index + 1}. 튜토리얼 단계 ${index + 1}** — 진행 중`,
      `  설명 ${index + 1}: 카드 하나에 다 보이면 너무 깁니다.`,
      `  다음 조작: /rpg 튜토리얼 ${index + 1}`
    ]).flat(),
    '보상 가능한 단계는 아래 선택 메뉴로 바로 수령하세요.'
  ].join('\n');

  const payload = createRpgVisualPayload(content);
  const description = payload.embeds[0].data.description;

  assert.doesNotMatch(description, /생략/);
});
...
test('RPG 튜토리얼은 신규 유저를 첫 전투와 보상 수령까지 안내한다', async () => {
  ...
  try {
    ...
    assert.ok(getReplyText(beforeStart.replies[0]).split('\n').filter((line) => line.trim()).length <= 5);
    ...
    assert.equal(battleButton.updates.length, 0);
    assertRpgEmbedCard(battleButton.replies[0], /RPG 전투 결과/);
...
test('RPG 빠른 버튼은 주요 화면과 다음 행동을 명령어 없이 이어준다', async () => {
  ...
  try {
    ...
    const battleButton = createRpgButtonInteraction('rpg_quick:user-1:battle');
    await handleRpgCommand(battleButton, fixture.economy);
    const exploreButton = createRpgButtonInteraction('rpg_quick:user-1:explore');
    await handleRpgCommand(exploreButton, fixture.economy);
    const dungeonButton = createRpgButtonInteraction('rpg_quick:user-1:dungeon');
    await handleRpgCommand(dungeonButton, fixture.economy);
    const raidButton = createRpgButtonInteraction('rpg_quick:user-1:raid');
    await handleRpgCommand(raidButton, fixture.economy);
    ...
    for (const payload of [
      ...
      battleButton.replies[0],
      exploreButton.replies[0],
      dungeonButton.replies[0],
      raidButton.replies[0]
    ]) {
      assertComponentRowsWithinDiscordLimit(payload);
    }
    ...
    assert.equal(battleButton.updates.length, 0);
    assert.equal(exploreButton.updates.length, 0);
    assert.equal(dungeonButton.updates.length, 0);
    assert.equal(raidButton.updates.length, 0);
    assert.match(battleButton.replies[0].embeds[0].data.title, /RPG 전투/);
    assert.match(exploreButton.replies[0].embeds[0].data.title, /RPG 탐험/);
    assert.match(dungeonButton.replies[0].embeds[0].data.title, /RPG 던전/);
    assert.match(raidButton.replies[0].embeds[0].data.title, /RPG 레이드/);
    assert.match(getReplyText(battleButton.replies[0]), /전투 판정/);
    assert.match(getReplyText(battleButton.replies[0]), /다음 추천/);
    assert.ok(
      battleButton.replies[0].components
