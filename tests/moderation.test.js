...
test('로그 채널과 금칙어 설정을 저장한다', async () => {
  ...
  try {
    ...
    assert.equal(settings.autoSpamBan.messageLimit, 8);
    assert.equal(settings.autoSpamBan.duplicateLimit, 5);
    assert.equal(settings.autoSpamBan.userSlowmodeDurationMs, parseDuration('5분'));
    assert.equal(settings.autoSpamBan.banAfterOffenses, null);
...
test('짧은 시간에 같은 메시지를 반복하면 슬로우모드만 반환하고 자동 밴하지 않는다', async () => {
  const fixture = await createFixture();
  ...
  try {
    ...
    const fourth = await fixture.moderation.recordMessageAndDetectSpam(createSpamInput(4, '도배'));
    const fifth = await fixture.moderation.recordMessageAndDetectSpam(createSpamInput(5, '도배'));
    await fixture.moderation.recordMessageAndDetectSpam(createSpamInput(6, '도배'));
    await fixture.moderation.recordMessageAndDetectSpam(createSpamInput(7, '도배'));
    await fixture.moderation.recordMessageAndDetectSpam(createSpamInput(8, '도배'));
    await fixture.moderation.recordMessageAndDetectSpam(createSpamInput(9, '도배'));
    const repeated = await fixture.moderation.recordMessageAndDetectSpam(createSpamInput(10, '도배'));

    assert.equal(first.detected, false);
    assert.equal(second.detected, false);
    assert.equal(third.detected, false);
    assert.equal(fourth.detected, false);
    assert.equal(fifth.detected, true);
    assert.deepEqual(fifth.punishment, {
      action: 'slowmode',
      durationMs: parseDuration('5분')
    });
    assert.equal(fifth.offenseCount, 1);
    assert.deepEqual(repeated.punishment, {
      action: 'slowmode',
      durationMs: parseDuration('5분')
    });
    assert.equal(repeated.offenseCount, 2);
    assert.match(fifth.reason, /같은 메시지 5회 반복/);
  } finally {
    await fixture.cleanup();
  }
});
...
test('짧은 시간에 메시지를 너무 많이 보내면 먼저 슬로우모드를 반환한다', async () => {
  ...
  try {
    ...
    assert.match(result.reason, /5초 내 메시지 8개/);
  } finally {
    await fixture.cleanup();
  }
});

test('도배 자동 대응은 채널 전체가 아니라 해당 유저만 5분 슬로우모드로 제한한다', async () => {
  const fixture = await createFixture();
...
test('기존 자동 도배 기본값은 더 느슨한 설정으로 자동 완화된다', async () => {
  const fixture = await createFixture();
  ...
  try {
          ...
          moderation: {
            settings: {
              ...
              autoSpamBan: {
                ...
                messageLimit: 5,
                duplicateLimit: 3,
                userSlowmodeDurationMs: parseDuration('10분'),
                banAfterOffenses: 2,
                offenseResetMs: parseDuration('10분')
              }
            },
            warnings: {},
            spam: {}
    ...
    const settings = await fixture.moderation.getSettings('guild-1');

    assert.equal(settings.autoSpamBan.messageLimit, 8);
    assert.equal(settings.autoSpamBan.duplicateLimit, 5);
    assert.equal(settings.autoSpamBan.userSlowmodeDurationMs, parseDuration('5분'));
    assert.equal(settings.autoSpamBan.banAfterOffenses, null);
    assert.equal(settings.autoSpamBan.offenseResetMs, parseDuration('5분'));
  } finally {
    await fixture.cleanup();
  }
});
