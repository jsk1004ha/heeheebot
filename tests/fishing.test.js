...
test('낚시 명령 payload는 요청한 명령들을 등록한다', () => {
  ...
  const assetCommand = payloads.find((command) => command.name === '낚시에셋');

  assert.deepEqual(names, ['낚시', '낚시강화', '잠수', '물고기팀설정', '물고기배틀', '낚시에셋']);
  ...
  assert.equal(assetCommand.options.length, 3);
});
