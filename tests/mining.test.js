...
test('광산 카탈로그는 50종 광석, 100강 곡괭이, 초단위 채굴 간격을 제공한다', () => {
  ...
  assert.equal(getOreConfig('stone_fragment').priceMin, 1);
...
test('광산 광물 시세 하한은 일반 1~50골드 기준으로 희귀도별 계단을 유지한다', () => {
  assert.equal(getOreConfig('stone_fragment').priceMin, 1);
  assert.equal(getOreConfig('silver_ore').priceMin, 70);
  assert.equal(getOreConfig('gold_ore').priceMin, 350);
  assert.equal(getOreConfig('mithril_ore').priceMin, 2_000);
  assert.equal(getOreConfig('adamantite_ore').priceMin, 40_000);
  assert.equal(getOreConfig('hidden_dawn_core').priceMin, 500_000);
});

test('광산 발견 XP는 고등급 광석 잭팟 가치와 분리해 크게 축소한다', () => {
  assert.equal(getOreDiscoveryXp('stone_fragment'), 1);
  assert.equal(getOreDiscoveryXp('silver_ore'), 9);
  assert.equal(getOreDiscoveryXp('gold_ore'), 31);
  assert.equal(getOreDiscoveryXp('mithril_ore'), 160);
  assert.equal(getOreDiscoveryXp('worldroot_fossil'), 4_400);
  assert.equal(getOreDiscoveryXp('hidden_dawn_core'), 25_000);
});
...
test('전설 광석은 +40 곡괭이부터 희귀도 롤에 들어간다', async () => {
  ...
  try {
    ...
    assert.equal(locked.rarity, 'epic');
    assert.equal(unlocked.rarity, 'legendary');
    assert.equal(unlocked.oreId, 'worldroot_fossil');
  } finally {
...
test('고레벨 채굴 희귀도 가중치는 영웅, 전설, 히든 확률을 낮춘 새 총량을 쓴다', async () => {
  const randomCalls = [];
  ...
  try {
    ...
    const result = await fixture.mining.mineOre(createInput({ now: 100_000, ignoreCooldown: true }));

    assert.equal(result.rarity, 'common');
    assert.deepEqual(randomCalls[0], { min: 1, max: 9_879 });
  } finally {
    await fixture.cleanup();
  }
});
...
test('광산 채굴 대기 카드는 데스크탑에서도 맨션이 보이도록 본문에 유저 멘션을 둔다', async () => {
  const fixture = await createFixture({ randomInt: minRandom });

  try {
    await handleMiningCommand(createInteraction('광산'), fixture.mining);
    const interaction = createInteraction('광산');
    await handleMiningCommand(interaction, fixture.mining);
    const payload = interaction.replies[0];

    assert.match(payload.embeds[0].data.title, /광산 채굴 대기/);
    assert.doesNotMatch(payload.embeds[0].data.title, /<@/);
    assert.match(payload.embeds[0].data.description, /<@user-1>/);
    assert.match(payload.embeds[0].data.description, /다음 채굴까지/);
  } finally {
    await fixture.cleanup();
  }
});
...
test('곡괭이 강화 명령은 100강 에셋과 골드 비용을 연결한다', async () => {
  ...
  try {
    ...
    assert.match(payload.embeds[0].data.description, /79,200골드/);
    assert.match(payload.embeds[0].data.description, /20,800골드/);
    assert.equal(payload.embeds[0].data.image.url, 'attachment://icon.png');
