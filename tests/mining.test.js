...
test('광산 카탈로그는 50종 광석, 100강 곡괭이, 초단위 채굴 간격을 제공한다', () => {
  ...
  assert.equal(getActiveMineCooldownMs(), 8_000);
...
test('광석 첫 발견은 도감 등록과 XP 보상을 한 번만 지급한다', async () => {
  ...
  try {
    ...
    const repeated = await fixture.mining.mineOre(createInput({ now: 28_000 }));
...
test('광석 판매는 UI 판매용 preview를 만들고 주식 파산 부채 자동상환과 연결된다', async () => {
  ...
  try {
    ...
    assert.equal(result.gross, 1_000);
    assert.equal(result.receipt.repayment, 250);
    assert.equal(result.receipt.net, 750);
    assert.equal(result.receipt.bankruptcy.debt, 750);
    assert.equal(result.receipt.balance, 750);
    assert.equal(data.mining.users['user-1'].inventory.hidden_dawn_core, undefined);
    assert.equal(data.accounts.users['user-1'].stockBankruptcy.debt, 750);
  } finally {
    await fixture.cleanup();
  }
});
