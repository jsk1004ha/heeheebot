...
test('대장장이 멘트는 같은 결과와 검 이름에서도 랜덤 후보를 가진다', () => {
  const result = {
    outcome: 'success',
    mode: 'advanced',
    beforeLevel: 0,
  ...
  const first = getBlacksmithEnhancementMessage(result, 0);
  const second = getBlacksmithEnhancementMessage(result, 0.99);

  assert.notEqual(first, second);
  assert.match(first, /바람결에 첫 예기를 드러낸 청동 수호검|제련석|태어났다/);
  assert.match(second, /바람결에 첫 예기를 드러낸 청동 수호검|제련석|강화/);
});
...
test('검강화 실패 계열 응답은 대장장이 위로와 놀림 메시지를 구분한다', async () => {
  ...
  assert.match(destroyed.content, /미안|재가|터졌|울지|처음부터|장례식|숯|조문|가루|무너졌|불꽃|아프다|조용|고강화/);
...
test('상급강화 대성공 응답은 제련석과 추가 확률을 게임식 문장으로 보여준다', async () => {
  ...
  assert.match(reply.content, /제련석 열기|한 번에 이렇게 뛰는|제련석을 통째로|새 검처럼/);
});
...
test('고강화 성공과 파괴는 대장장이 특수 멘트를 우선한다', async () => {
  ...
  assert.match(hundred.content, /서버 전체|전설|완성품|희희검|망치 인생/);
  assert.match(funeral.content, /장례식|숯|조문|고강화|대장간|조용/);
});
