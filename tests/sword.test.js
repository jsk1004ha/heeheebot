...
test('검 명령 payload는 강화, 상급강화, 판매, 배틀, 선물받기를 등록한다', () => {
  ...
  assert.equal(payloads[3].options[0].name, '상대');
  assert.equal(payloads[3].options[0].required, false);
});
...
test('대장장이 이미지 에셋은 검강화 응답용 PNG로 제공된다', () => {
  const blacksmithPath = join(process.cwd(), 'assets', 'sword', 'blacksmith', 'blacksmith.png');
...
test('검강화 명령 응답은 강화 단계 이미지와 대장장이 축하 메시지를 포함한다', async () => {
  ...
  assert.equal(replies[0].files.some((file) => file.name === 'blacksmith.png'), true);
  assert.equal(replies[0].embeds[0].data.image.url, 'attachment://sword_001.png');
  assert.equal(replies[0].embeds[0].data.thumbnail.url, 'attachment://blacksmith.png');
});
...
test('검강화 실패 계열 응답은 대장장이 위로와 놀림 메시지를 구분한다', async () => {
  ...
  assert.match(destroyed.content, /미안|재가|터졌|울지|처음부터/);
});

test('검판매 명령은 현재 검을 돈으로 바꾸고 판매한 검 이미지로 응답한다', async () => {
  const replies = [];
  const interaction = createSwordInteraction({
    commandName: '검판매',
    replies
  ...
  assert.equal(handled, true);
  assert.equal(replies.length, 1);
  assert.match(replies[0].content, /검판매/);
  assert.match(replies[0].content, /판매 금액: \*\*750원\*\*/);
  assert.match(replies[0].content, /현재 검: \*\*\+0 기본 검\*\*/);
  assert.equal(replies[0].files[0].name, 'sword_012.png');
  assert.equal(replies[0].embeds[0].data.title, '판매한 검 — +12 흐린 별빛을 머금은 방랑자의 검');
  assert.equal(replies[0].embeds[0].data.image.url, 'attachment://sword_012.png');
});
...
function createSwordInteraction({ commandName, replies, target = null }) {
  return {
    commandName,
    guildId: 'guild-1',
    user: {
