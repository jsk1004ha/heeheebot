...
test('애플리케이션 명령 레지스트리는 44fa961 추가 명령을 모두 포함한다', () => {
  ...
  assert.ok(commandNames.includes('돈갚기'), '간단 상환 명령은 실제 등록 payload에 있어야 합니다.');
  assert.equal(commandNames.includes('노드상태'), false, '진단용 노드상태는 100개 제한 때문에 공개 등록에서 제외합니다.');
  assert.equal(commandNames.includes('자동급식'), false, '저빈도 자동 급식 설정은 100개 제한 때문에 공개 등록에서 제외합니다.');
