...
test('마피아게임 로비는 4~16명 제한과 직업별 비공개 회의실을 적용한다', async () => {
  ...
  assert.equal(channel.createdThreads.length, 2);
  assert.deepEqual(channel.createdThreads.map((thread) => thread.name.split(' ')[0]).sort(), ['마피아', '의사']);

  const startPayload = channel.sent.find((payload) => typeof payload === 'object' && payload.content?.includes('마피아게임 시작'));
  const nightPayload = channel.sent.find((payload) => typeof payload === 'object' && payload.content?.includes('일차 밤'));
  ...
  await handleMafiaCommand(createButtonInteraction({ ...base, customId: roleButtonId, replies }), null, console);
  ...
  const actor = await findNightActor({ base, users, nightButtonId, replies });
