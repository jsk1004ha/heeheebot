...
test('마피아게임 로비는 4~16명 제한과 직업별 비공개 회의실을 적용한다', async () => {
  ...
  const nightPayload = gameThread.sent.find((payload) => typeof payload === 'object' && payload.content?.includes('일차 밤'));
  ...
  assert.ok(gameThread.sent.some((payload) => typeof payload === 'string' && payload.includes('밤 결과')));
