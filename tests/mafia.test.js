...
test('마피아게임 역할 분포는 4~12명 밸런스 표를 따른다', () => {
  ...
  assert.deepEqual(getMafiaRoleDistribution(12), { mafia: 2, police: 2, doctor: 2, citizen: 6 });
  assert.throws(() => getMafiaRoleDistribution(3), /4~12명/);
  assert.throws(() => getMafiaRoleDistribution(13), /4~12명/);
});
...
test('마피아게임 재판은 투표, 최후의 반론, 찬반 투표, 동률/부결을 처리한다', () => {
  const game = new MafiaGame({
  ...
  assert.equal(game.castVote({ voterId: 'user-1', targetId: 'user-1' }).accepted, true);
  game.clearVotes();
  assert.equal(game.resolveNomination().reason, 'no_votes');
  ...
  const nomination = game.resolveNomination();
  assert.equal(nomination.candidate.userId, 'user-1');
  assert.equal(game.getPlayer('user-1').alive, true);
  assert.equal(game.castApprovalVote({ voterId: 'user-1', approve: true }).accepted, false);
  ...
  rejected.castVote({ voterId: 'user-2', targetId: 'user-1' });
  rejected.castVote({ voterId: 'user-3', targetId: 'user-1' });
  rejected.castVote({ voterId: 'user-4', targetId: 'user-1' });
  assert.equal(rejected.resolveNomination().candidate.userId, 'user-1');
  rejected.castApprovalVote({ voterId: 'user-2', approve: false });
  rejected.castApprovalVote({ voterId: 'user-3', approve: false });
  rejected.castApprovalVote({ voterId: 'user-4', approve: false });
  const rejectedResult = rejected.resolveApprovalVote();
  assert.equal(rejectedResult.reason, 'rejected');
  assert.equal(rejectedResult.noExecution, true);
  assert.equal(rejected.getPlayer('user-1').alive, true);
  ...
  const tieResult = tie.resolveNomination();
  assert.equal(tieResult.noCandidate, true);
  assert.equal(tieResult.candidate, null);
});

test('마피아게임 로비는 4~12명 제한과 직업별 비공개 회의실을 적용한다', async () => {
  resetMafiaGamesForTest();
  ...
  const discussionPayload = gameThread.sent.find((payload) => typeof payload === 'object' && payload.content?.includes('낮 토론 시간'));
  assert.ok(discussionPayload);
  assert.match(discussionPayload.content, /투표, 최후의 반론, 찬반 투표/);
  const discussionSkipId = findButtonId(discussionPayload, '방장 스킵');
  await handleMafiaCommand(createButtonInteraction({ ...threadBase, customId: discussionSkipId, replies }), null, console);

  const votePayload = gameThread.sent.find((payload) => getEmbedTitle(payload)?.includes('일차 투표'));
  assert.ok(votePayload);
  assert.match(getEmbedFooter(votePayload), /최다 득표자/);
  const voteUser1Id = findButtonId(votePayload, '유저1');
  await handleMafiaCommand(createButtonInteraction({ ...threadBase, user: users[1], customId: voteUser1Id, updates, replies }), null, console);
  await handleMafiaCommand(createButtonInteraction({ ...threadBase, user: users[2], customId: voteUser1Id, updates, replies }), null, console);
  const voteSkipId = findButtonId(votePayload, '방장 스킵');
  await handleMafiaCommand(createButtonInteraction({ ...threadBase, customId: voteSkipId, replies }), null, console);

  const defensePayload = gameThread.sent.find((payload) => typeof payload === 'object' && payload.content?.includes('**1일차 최후의 반론**'));
  assert.ok(defensePayload);
  assert.match(defensePayload.content, /user-1/);
  const defenseSkipId = findButtonId(defensePayload, '방장 스킵');
  await handleMafiaCommand(createButtonInteraction({ ...threadBase, customId: defenseSkipId, replies }), null, console);

  const approvalPayload = gameThread.sent.find((payload) => getEmbedTitle(payload)?.includes('찬반 투표'));
  assert.ok(approvalPayload);
  const approveId = findButtonId(approvalPayload, '찬성');
  await handleMafiaCommand(createButtonInteraction({ ...threadBase, user: users[1], customId: approveId, updates, replies }), null, console);
  await handleMafiaCommand(createButtonInteraction({ ...threadBase, user: users[2], customId: approveId, updates, replies }), null, console);
  await handleMafiaCommand(createButtonInteraction({ ...threadBase, user: users[3], customId: approveId, updates, replies }), null, console);
  await handleMafiaCommand(createButtonInteraction({ ...threadBase, user: users[6], customId: approveId, updates, replies }), null, console);

  assert.ok(gameThread.sent.some((payload) => typeof payload === 'string' && payload.includes('처형 결과')));
  assert.deepEqual(gameThread.removedMembers.sort(), ['user-1', 'user-5', 'user-6']);
...
function findButtonId(payload, label) {
  for (const row of payload.components ?? []) {
    const components = typeof row.toJSON === 'function' ? row.toJSON().components : row.components;
    const button = components?.find((component) => component.label === label);
    if (button) return button.custom_id ?? button.customId;
  }
  return null;
}

function getEmbedTitle(payload) {
  return payload?.embeds?.[0]?.data?.title ?? payload?.embeds?.[0]?.title ?? null;
}

function getEmbedFooter(payload) {
  return payload?.embeds?.[0]?.data?.footer?.text ?? payload?.embeds?.[0]?.footer?.text ?? '';
}
