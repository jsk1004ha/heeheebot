...
test('라이어게임 명령 payload는 시작 옵션과 10개 이상 카테고리를 등록한다', async () => {
  ...
  const startCommand = payload.options.find((option) => option.name === '시작');
  const submitCommand = payload.options.find((option) => option.name === '제출');

  assert.equal(payload.name, '라이어게임');
  assert.ok(startCommand);
  assert.ok(submitCommand);
  assert.deepEqual(startCommand.options.find((option) => option.name === '모드').choices.map((choice) => choice.value), [
    LIAR_GAME_MODES.normal.value,
    LIAR_GAME_MODES.hard.value
  ]);
  assert.deepEqual(startCommand.options.find((option) => option.name === '설명턴수').choices.map((choice) => choice.value), [1, 2]);
  assert.equal(submitCommand.options.find((option) => option.name === '내용').required, true);
...
test('라이어게임 설명은 일반 채팅이 아니라 제출 명령어로만 처리한다', async () => {
  resetLiarGameSessionsForTest();
  ...
  assert.equal(await handleLiarGameMessage(createMessage({ ...base, content: '자주 먹는 음식입니다.' }), null, console), false);
  assert.deepEqual(channel.reactions, []);

  assert.equal(await handleLiarGameCommand(createSlashInteraction({
    ...base,
    replies,
    subcommand: '제출',
    options: { 내용: '자주 먹는 음식입니다.' }
  }), null, console), true);
  assert.match(replies.at(-1).content, /설명 제출/);
  assert.match(replies.at(-1).content, /자주 먹는 음식입니다/);
...
function createSlashInteraction({ guildId, channelId, channel, user, replies, options, subcommand = '시작' }) {
  return {
    commandName: '라이어게임',
    guildId,
    channelId,
