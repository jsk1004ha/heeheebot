...
test('거대 골드 잔액은 프로필 로드와 지급 후에도 문자열 정밀도를 유지한다', async () => {
  const fixture = await createFixture({ dailyReward: 2, dailyXpReward: 0 });
      ...
      data.guilds['guild-1'] = {
        users: {
          'user-1': {
            ...
            xp: 0,
            totalXp: 0,
            balance: '900719925474099199999'
          }
        }
      };
    });

    const before = await fixture.economy.getProfile('guild-1', 'user-1', '거대잔액');
    const reward = await fixture.economy.claimDaily({
      guildId: 'guild-1',
      userId: 'user-1',
      username: '거대잔액',
      now: 1000
    });
    const after = await fixture.economy.getProfile('guild-1', 'user-1', '거대잔액');

    assert.equal(before.balance, '900719925474099199999');
    assert.equal(reward.profile.balance, '900719925474099200001');
    assert.equal(after.balance, '900719925474099200001');
  } finally {
    await fixture.cleanup();
  }
});

test('랭킹은 safe integer를 넘는 골드도 정확하게 비교한다', async () => {
  const fixture = await createFixture();
  ...
  try {
    ...
    assert.equal(leaderboard[0].userId, 'higher');
    assert.equal(leaderboard[0].balance, '900719925474099200001');
  } finally {
    await fixture.cleanup();
  }
});
...
test('송금은 safe integer 초과 금액도 정밀하게 이동한다', async () => {
  const fixture = await createFixture();
  ...
  try {
    ...
    const transfer = await fixture.economy.transfer({
      ...
      toUserId: 'receiver',
      toUsername: '받는거액',
      amount: '900719925474099200000'
    });

    assert.equal(transfer.amount, '900719925474099200000');
    assert.equal(transfer.from.balance, 1);
    assert.equal(transfer.to.balance, '900719925474099200000');
  } finally {
    await fixture.cleanup();
  }
});
...
test('유저 대출은 조건 제시와 최종 수락 후 만기 뒤 수익 35%를 빌려준 유저에게 자동 상환한다', async () => {
  ...
  try {
    ...
    assert.equal(offer.offer.interestPeriodMs, 24 * 60 * 60 * 1000);
    assert.equal(offer.offer.totalDue, 1_100);
    assert.equal(accepted.borrower.balance, 1_000);
    assert.equal(accepted.lender.balance, 999_000);
    ...
    assert.equal(lenderAfterAuto.balance, 999_350);
    ...
    assert.equal(finalLender.balance, 1_000_100);
  } finally {
    await fixture.cleanup();
  }
});

test('유저 대출은 빌려준 사람의 진행 중 원금 총액을 보유 골드 0.1%로 제한한다', async () => {
  const fixture = await createFixture();
  ...
  try {
    ...
    await assert.rejects(
      () => fixture.economy.requestUserLoan({
        guildId: 'guild-1',
        borrowerUserId: 'borrower',
        borrowerUsername: '빌리는사람',
        lenderUserId: 'lender',
    ...
    await fixture.economy.offerUserLoan({
      guildId: 'guild-1',
      lenderUserId: 'lender',
      lenderUsername: '빌려준사람',
      borrowerUserId: 'borrower',
    ...
    assert.equal(accepted.loan.principal, 1_000);
    await assert.rejects(
      () => fixture.economy.requestUserLoan({
        guildId: 'guild-1',
        borrowerUserId: 'borrower2',
        borrowerUsername: '또빌리는사람',
        lenderUserId: 'lender',
        lenderUsername: '빌려준사람',
        amount: 1,
        termHours: 1,
...
test('유저 대출 최종 결정은 빌려준 사람 잔액 변화 뒤에도 0.1% 한도를 다시 확인한다', async () => {
  const fixture = await createFixture();
  ...
  try {
    ...
    await fixture.store.update((data) => {
      data.accounts.users.lender.balance = 999_000;
    });

    await assert.rejects(
      () => fixture.economy.decideUserLoan({
        guildId: 'guild-1',
        borrowerUserId: 'borrower',
        borrowerUsername: '빌리는사람',
        lenderUserId: 'lender',
...
test('돈빌려주기 승인 흐름은 요청 조건 그대로 실행하고 이자는 지난 주기만큼만 붙인다', async () => {
  const fixture = await createFixture();
  ...
  try {
    ...
    const request = await fixture.economy.requestUserLoan({
      ...
      interestPercent: 10,
      interestPeriodHours: 3,
      interestType: 'compound',
      now: 1_000
    });
    const accepted = await fixture.economy.acceptUserLoanRequest({
      guildId: 'guild-1',
      lenderUserId: 'lender',
      lenderUsername: '빌려준사람',
      borrowerUserId: 'borrower',
    ...
    assert.equal(request.request.interestBps, 1_000);
    assert.equal(request.request.interestPeriodMs, 3 * hour);
    assert.equal(request.request.interestType, 'compound');
    assert.equal(accepted.loan.principal, 10_000);
    assert.equal(accepted.loan.totalDue, 10_000);
    assert.equal(accepted.borrower.balance, 10_000);

    const earlyRepay = await fixture.economy.repayUserLoan({
      guildId: 'guild-1',
      borrowerUserId: 'borrower',
      borrowerUsername: '빌리는사람',
      lenderUserId: 'lender',
    ...
    assert.equal(earlyRepay.repaid, 10_000);
    assert.equal(earlyRepay.remaining, 0);
    assert.equal(earlyRepay.borrower.balance, 0);
    assert.equal(earlyRepay.lender.balance, 20_000_000);
  } finally {
    await fixture.cleanup();
  }
});

test('대출 이자는 선택한 이자주기가 지날 때마다 복리로 증가한다', async () => {
  const fixture = await createFixture();
  const hour = 60 * 60 * 1000;
  ...
  try {
    ...
    await fixture.economy.acceptUserLoanRequest({
      guildId: 'guild-1',
      lenderUserId: 'lender',
      lenderUsername: '빌려준사람',
      borrowerUserId: 'borrower',
    ...
    await fixture.store.update((data) => {
      data.accounts.users.borrower.balance = 20_000;
    });
    ...
    assert.equal(repaid.repaid, 12_100);
    assert.equal(repaid.remaining, 0);
    assert.equal(repaid.borrower.balance, 7_900);
    assert.equal(repaid.lender.balance, 20_002_100);
  } finally {
    await fixture.cleanup();
  }
});

test('유저 대출은 safe integer 초과 원금과 상환액을 정밀하게 보존한다', async () => {
  const fixture = await createFixture();
  const hour = 60 * 60 * 1000;
  const lenderInitial = 1_000_000_000_000_000_000_000_000n;
  const principal = 900_719_925_474_099_200_000n;
  const totalDue = principal + (principal / 10n);
  ...
  try {
      ...
      data.guilds['guild-1'] = {
        users: {
          ...
          lender: {
            ...
            xp: 0,
            totalXp: 0,
            balance: lenderInitial.toString()
          }
        }
      };
    });
    ...
    assert.equal(offer.offer.totalDue, totalDue.toString());
    assert.equal(accepted.borrower.balance, principal.toString());
    assert.equal(accepted.lender.balance, (lenderInitial - principal).toString());

    await fixture.store.update((data) => {
      data.accounts.users.borrower.balance = totalDue.toString();
    });
    ...
    assert.equal(repaid.repaid, totalDue.toString());
    assert.equal(repaid.remaining, 0);
    assert.equal(repaid.borrower.balance, 0);
    assert.equal(repaid.lender.balance, (lenderInitial - principal + totalDue).toString());
  } finally {
    await fixture.cleanup();
  }
});

test('유저 대출 직접 상환은 금액 생략 시 가진 만큼 갚을 수 있다', async () => {
  const fixture = await createFixture();
  ...
  try {
    ...
    await fixture.economy.decideUserLoan({
      guildId: 'guild-1',
      borrowerUserId: 'borrower',
      borrowerUsername: '빌리는사람',
      lenderUserId: 'lender',
    ...
    await fixture.store.update((data) => {
      const borrower = data.accounts?.users?.borrower ?? data.guilds['guild-1']?.users?.borrower;
      assert.ok(borrower);
      borrower.balance = 400;
    });
    ...
    assert.equal(repaid.repaid, 400);
    assert.equal(repaid.remaining, 600);
    assert.equal(repaid.borrower.balance, 0);
    assert.equal(finalLender.balance, 999_400);
  } finally {
    await fixture.cleanup();
  }
});

test('유저 대출은 빌려주는 사람이 정한 이자 기간마다 단리와 복리를 계산한다', async () => {
  const fixture = await createFixture();
  ...
  try {
    ...
    await fixture.economy.requestUserLoan({
      ...
      borrowerUserId: 'borrower',
      borrowerUsername: '빌리는사람',
      lenderUserId: 'simpleLender',
      lenderUsername: '단리대출자',
      amount: 1_000,
      termHours: 6,
      repaymentMode: 'lump_sum',
      now: 1_000
    });
    const simpleOffer = await fixture.economy.offerUserLoan({
      guildId: 'guild-1',
      lenderUserId: 'simpleLender',
      lenderUsername: '단리대출자',
      borrowerUserId: 'borrower',
    ...
    await fixture.economy.requestUserLoan({
      ...
      borrowerUserId: 'borrower',
      borrowerUsername: '빌리는사람',
      lenderUserId: 'compoundLender',
      lenderUsername: '복리대출자',
      amount: 1_000,
      termHours: 6,
      repaymentMode: 'lump_sum',
      now: 3_000
    });
    const compoundOffer = await fixture.economy.offerUserLoan({
      guildId: 'guild-1',
      lenderUserId: 'compoundLender',
      lenderUsername: '복리대출자',
      borrowerUserId: 'borrower',
    ...
    assert.equal(simpleOffer.offer.interestPeriodMs, 2 * 60 * 60 * 1000);
    assert.equal(simpleOffer.offer.totalDue, 1_300);
    assert.equal(compoundOffer.offer.interestPeriodMs, 2 * 60 * 60 * 1000);
    assert.equal(compoundOffer.offer.totalDue, 1_331);
  } finally {
    await fixture.cleanup();
  }
});

test('유저 대출 현황은 빌린 대출과 빌려준 대출을 함께 조회한다', async () => {
  const fixture = await createFixture();
  const hour = 60 * 60 * 1000;
  ...
  try {
    ...
    assert.equal(beforeAccept.incomingRequests.length, 1);
    assert.equal(beforeAccept.incomingRequests[0].borrowerUserId, 'borrower');
    assert.equal(beforeAccept.incomingRequests[0].status, 'requested');
    ...
    const borrowerStatus = await fixture.economy.getUserLoanStatus({
      ...
      username: '빌리는사람',
      targetUserId: 'lender',
      targetUsername: '빌려준사람',
      now: 3_000 + (6 * hour)
    });

    assert.equal(lenderStatus.incomingRequests.length, 0);
    assert.equal(lenderStatus.lentLoans.length, 1);
    assert.equal(lenderStatus.lentLoans[0].borrowerUserId, 'borrower');
    assert.equal(lenderStatus.lentLoans[0].remaining, 1_331);
    assert.equal(borrowerStatus.borrowedLoans.length, 1);
    assert.equal(borrowerStatus.borrowedLoans[0].lenderUserId, 'lender');
    assert.equal(borrowerStatus.borrowedLoans[0].interestPeriodMs, 2 * 60 * 60 * 1000);
    assert.equal(borrowerStatus.target.userId, 'lender');
  } finally {
    await fixture.cleanup();
  }
});
...
test('유저 대출은 만기 후에도 이자 기간이 지나면 추가 이자가 붙는다', async () => {
  ...
  try {
    ...
    const simpleDueAt = 3_000 + 2 * hour;
    const compoundDueAt = 6_000 + 2 * hour;
    const beforeExtra = await fixture.economy.getUserLoanStatus({
      guildId: 'guild-1',
      userId: 'borrower',
      username: '빌리는사람',
      targetUserId: 'simpleLender',
      targetUsername: '단리대출자',
      now: simpleDueAt + hour - 1
    });
    const afterExtra = await fixture.economy.getUserLoanStatus({
      guildId: 'guild-1',
      userId: 'borrower',
      username: '빌리는사람',
      now: compoundDueAt + hour
    });
    const simpleLoan = afterExtra.borrowedLoans.find((loan) => loan.lenderUserId === 'simpleLender');
    const compoundLoan = afterExtra.borrowedLoans.find((loan) => loan.lenderUserId === 'compoundLender');

    assert.equal(beforeExtra.borrowedLoans[0].totalDue, 1_200);
    assert.equal(simpleLoan.totalDue, 1_300);
    assert.equal(simpleLoan.remaining, 1_300);
    assert.equal(compoundLoan.totalDue, 1_331);
    assert.equal(compoundLoan.remaining, 1_331);
  } finally {
    await fixture.cleanup();
  }
});
...
test('마피아게임 결과 보상은 승리 팀만 골드를 받고 패배 팀은 경험치만 받는다', async () => {
  const fixture = await createFixture({
    mafiaGameWinnerXp: 160,
    mafiaGameWinnerMoney: 900,
    mafiaGameLoserXp: 35,
    levelBaseXp: 1000
  });
  const participants = [
    { userId: 'mafia-1', username: '마피아', role: 'mafia' },
    { userId: 'police-1', username: '경찰', role: 'police' },
    { userId: 'doctor-1', username: '의사', role: 'doctor' },
    { userId: 'citizen-1', username: '시민', role: 'citizen' }
  ];

  try {
    const citizenWin = await fixture.economy.awardMafiaGameResults({
      guildId: 'guild-1',
      participants,
      winner: 'citizens'
    });
    ...
    assert.equal(citizenWin.winner, 'citizens');
    assert.deepEqual(citizenWin.participants.map((result) => result.xpGained), [35, 160, 160, 160]);
    assert.deepEqual(citizenWin.participants.map((result) => result.moneyGained), [0, 900, 900, 900]);
    assert.equal(citizenWin.winners.length, 3);
    assert.equal(citizenWin.losers.length, 1);

    const mafiaWin = await fixture.economy.awardMafiaGameResults({
      guildId: 'guild-1',
      participants,
      winner: 'mafia'
    });

    assert.equal(mafiaWin.winner, 'mafia');
    assert.deepEqual(mafiaWin.participants.map((result) => result.xpGained), [160, 35, 35, 35]);
    assert.deepEqual(mafiaWin.participants.map((result) => result.moneyGained), [900, 0, 0, 0]);
  } finally {
    await fixture.cleanup();
  }
});
