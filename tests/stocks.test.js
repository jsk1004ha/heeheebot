...
const JIMIN_LONG_SHORTFALL_DEBT = 2_900_000;
...
test('레버리지 포지션은 만기 전 변동을 무시하고 기간 끝 가격으로 자동 정산한다', async () => {
  await withFixture(async ({ stocks, store }) => {
    ...
    assert.equal(portfolio.bankruptcy.debt, 380);
    ...
    assert.equal(history.entries[0].bankruptcyDebtAdded, 380);
  }, { randomInt: deterministicRandomInt([100, 100]) });
});

test('레버리지 롱 청산 채무는 증거금 패널티가 아니라 차입 평가금 부족분으로 계산한다', async () => {
  await withFixture(async ({ stocks, store }) => {
    ...
    assert.equal(portfolio.settled[0].bankruptcyDebtAdded, JIMIN_LONG_SHORTFALL_DEBT);
    assert.equal(portfolio.bankruptcy.debt, JIMIN_LONG_SHORTFALL_DEBT);
...
test('파산채무가 남으면 새 레버리지 진입을 막고 이후 골드 수익에서 자동 상환한다', async () => {
  await withFixture(async ({ stocks, store }) => {
    ...
    assert.equal(liquidated.bankruptcy.debt, JIMIN_LONG_SHORTFALL_DEBT);
    ...
    assert.equal(repaid.bankruptcy.debt, JIMIN_LONG_SHORTFALL_DEBT - 250);
...
test('파산채무는 보유 골드로 직접 상환할 수 있고 전액 상환 후 레버리지 진입이 풀린다', async () => {
  await withFixture(async ({ stocks, store }) => {
    await createJiminLongShortfallDebtScenario({ stocks, store, balance: 3_100_000 });
    ...
    assert.equal(repaid.requested, 3_000_000);
    assert.equal(repaid.repaid, JIMIN_LONG_SHORTFALL_DEBT);
...
test('레버리지 명령 화면은 파산채무와 진입 제한을 안내한다', async () => {
  await withFixture(async ({ stocks, store }) => {
    ...
    assert.match(portfolioInteraction.replies[0], /파산채무: \*\*2,900,000골드\*\*/);
    ...
    assert.match(openInteraction.replies[0], /파산채무 2,900,000골드/);
