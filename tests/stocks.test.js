...
test('주식 확장 명령은 신규상장, 지정가 주문, 알림 응답을 출력한다', async () => {
  ...
  assert.match(newsInteraction.replies[0], /희진전자 신제품 공개 일정이 예고됐습니다/);
...
test('전체시세 응답은 실제 결과 길이와 상승/하락 색상 표시를 보여준다', async () => {
  ...
  assert.doesNotMatch(fullMarketInteraction.replies[0], /테스트주11/);
  assert.deepEqual(
  ...
  assert.match(updates[0].content, /테스트주11/);
  assert.match(updates[0].content, /테스트주12/);
  assert.doesNotMatch(updates[0].content, /희진전자/);
