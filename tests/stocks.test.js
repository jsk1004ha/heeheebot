...
test('뉴스 공시는 사전 원인으로 기록되고 다음 tick 가격에 반영된다', async () => {
  await withFixture(async ({ stocks }) => {
    ...
    assert.match(heejinNews.message, /시장 공시/);
    assert.doesNotMatch(heejinNews.message, /급등|급락|상장폐지|호재|악재|리스크|예상 영향|\+\d|-\d/);
...
test('상장폐지 위험 뉴스도 방향을 숨긴 사전 공시로 표현한다', async () => {
  await withFixture(async ({ stocks }) => {
    ...
    assert.match(riskNews.message, /시장 공시/);
    assert.doesNotMatch(riskNews.message, /급등|급락|상장폐지|호재|악재|리스크|우려|위험|예상 영향|\+\d|-\d/);
  }, { randomInt: (min) => min });
});
...
test('주식 확장 명령은 신규상장, 지정가 주문, 알림 응답을 출력한다', async () => {
        ...
        entries: [
          {
            ...
            title: '시장 공시',
            message: '시장 공시: 희진전자 신제품 공개 일정이 예고됐습니다',
            tickIndex: 8,
  ...
  assert.doesNotMatch(newsInteraction.replies[0], /급등|급락|호재|악재|리스크|예상 영향|\+\d|-\d/);
