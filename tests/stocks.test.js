...
import { getStockCommandPayloads } from '../src/commands/stocks.js';
...
test('시장은 tick 간격이 지나면 결정적 난수로 가격과 뉴스가 갱신된다', async () => {
  await withFixture(async ({ stocks }) => {
    ...
    const after = await stocks.getMarket({ guildId: 'guild-1', now: 30 * 60 * 1000 });
