...
const JIMIN_LONG_LIQUIDATION_DEBT = 12_900_000;
...
test('주식 명령 payload는 현물, 지정가, 알림, 배당금, 신규상장, 레버리지 subcommand를 등록한다', () => {
  ...
  const buyQuantityOption = buyCommand.options.find((option) => option.name === '수량');
  ...
  const sellQuantityOption = sellCommand.options.find((option) => option.name === '수량');
  const limitBuyCommand = payload.options.find((option) => option.name === '지정가매수');
  const limitBuyQuantityOption = limitBuyCommand.options.find((option) => option.name === '수량');
  const limitSellCommand = payload.options.find((option) => option.name === '지정가매도');
  const limitSellQuantityOption = limitSellCommand.options.find((option) => option.name === '수량');
  ...
  const leverageMarginOption = leverageCommand.options.find((option) => option.name === '증거금');
  ...
  for (const quantityOption of [buyQuantityOption, sellQuantityOption, limitBuyQuantityOption, limitSellQuantityOption]) {
    assert.equal(quantityOption.min_value, 1);
    assert.equal(quantityOption.max_value, undefined, '주식 수량은 1만 주 상한 없이 보유 골드/주식으로만 제한한다');
  }
...
test('주식 현물 매매는 safe integer 초과 골드 잔액을 보존한다', async () => {
  await withFixture(async ({ stocks, store }) => {
    const initialBalance = 900_719_925_474_099_200_000n;
    await seedBalance(store, 'guild-1', 'user-1', '희희', initialBalance.toString());
    ...
    assert.equal(buy.profile.balance, (initialBalance - 8_080n).toString());
    assert.equal(sell.profile.balance, (initialBalance - 4_912n).toString());
    assert.equal(portfolio.cash, (initialBalance - 4_912n).toString());
    assert.equal(portfolio.totalAssets, (initialBalance - 112n).toString());
  });
});
...
test('트리거된 가격 알림 발송 표시는 여러 서버 알림도 한 번의 저장 작업으로 처리한다', async () => {
  await withFixture(async ({ store, stocks }) => {
    const firstAlert = await stocks.setPriceAlert({
      guildId: 'guild-1',
      userId: 'user-1',
      username: '희희',
    ...
    await stocks.getPendingTriggeredPriceAlertsByGuild({
    ...
    const notified = await stocks.markPriceAlertsNotifiedBatch({
      alerts: [
        { guildId: 'guild-1', userId: 'user-1', alertId: firstAlert.id },
        { guildId: 'guild-2', userId: 'user-2', alertId: secondAlert.id },
        { guildId: 'guild-2', userId: 'user-2', alertId: secondAlert.id }
      ],
      now: 3 * 60 * 1000 + 1
    });
    ...
    assert.deepEqual(notified.map((alert) => alert.id).sort(), [firstAlert.id, secondAlert.id].sort());

    const pendingAfterNotified = await stocks.getPendingTriggeredPriceAlertsByGuild({
      guildIds: ['guild-1', 'guild-2'],
      now: 3 * 60 * 1000 + 2
    });

    assert.deepEqual(pendingAfterNotified['guild-1'], []);
    assert.deepEqual(pendingAfterNotified['guild-2'], []);
  }, { randomInt: deterministicRandomInt([100, 99]) });
});
...
test('레버리지 포지션은 기간별 배율 제한을 적용하고 증거금 수수료를 차감한다', async () => {
  await withFixture(async ({ stocks, store }) => {
    ...
    assert.equal(short.position.durationTurns, 2);
    ...
    const debtSummary = await stocks.getLeverageDebtSummary({
      guildId: 'guild-1',
      userId: 'user-1',
      username: '희희',
      now: 1
    });
    assert.equal(debtSummary.activeDebt, 1_020_000);
    assert.equal(debtSummary.bankruptcyDebt, 0);
    assert.equal(debtSummary.debtTotal, 1_020_000);
    assert.equal(debtSummary.positionCount, 2);
...
test('레버리지 포지션은 만기 전 변동을 무시하고 기간 끝 가격으로 자동 정산한다', async () => {
  await withFixture(async ({ stocks, store }) => {
    ...
    assert.equal(portfolio.bankruptcy.debt, 100_380);
    ...
    assert.equal(history.entries[0].bankruptcyDebtAdded, 100_380);
  }, { randomInt: deterministicRandomInt([100, 100]) });
});

test('레버리지 청산 채무는 차입금과 레버리지 손실을 합산한다', async () => {
  await withFixture(async ({ stocks, store }) => {
    const { opened, portfolio } = await createJiminLongShortfallDebtScenario({ stocks, store });

    assert.equal(opened.position.entryPrice, 490);
    assert.equal(opened.position.notional, 10_000_000);
    assert.equal(opened.position.debt, 9_900_000);
    assert.equal(portfolio.positions.length, 0);
    assert.equal(portfolio.settled[0].type, 'leverage_settlement');
    assert.equal(portfolio.settled[0].price, 343);
    assert.equal(portfolio.settled[0].bankruptcyDebtAdded, JIMIN_LONG_LIQUIDATION_DEBT);
    assert.equal(portfolio.bankruptcy.debt, JIMIN_LONG_LIQUIDATION_DEBT);
    assert.equal(portfolio.cash, 98_000);
  }, { randomInt: deterministicRandomInt([0, 99]) });
});

test('100원에서 50원으로 떨어진 롱 100배는 차입금 9,900원과 손실 5,000원을 모두 채무로 남긴다', async () => {
  await withFixture(async ({ stocks, store }) => {
    await seedBalance(store, 'guild-1', 'user-1', '희희', 1_000);
    const listed = await stocks.listAdminStock({
      guildId: 'guild-1',
      name: '백원전자',
      symbol: 'HUND',
      basePrice: 100,
    ...
    assert.equal(opened.position.entryPrice, 100);
    assert.equal(opened.position.debt, 9_900);
    assert.equal(portfolio.positions.length, 0);
    assert.equal(portfolio.settled[0].bankruptcyDebtAdded, 14_900);
    assert.equal(portfolio.bankruptcy.debt, 14_900);
  }, { randomInt: deterministicRandomInt([0, 99]) });
});

test('파산채무가 남으면 새 레버리지 진입을 막고 이후 골드 수익에서 자동 상환한다', async () => {
  await withFixture(async ({ stocks, store }) => {
    const { portfolio: liquidated } = await createJiminLongShortfallDebtScenario({ stocks, store });

    assert.equal(liquidated.bankruptcy.debt, JIMIN_LONG_LIQUIDATION_DEBT);
    ...
    assert.equal(repaid.bankruptcy.debt, JIMIN_LONG_LIQUIDATION_DEBT - 250);
    assert.equal(repaid.bankruptcy.paid, 250);
    assert.equal(repaid.cash, 98_750);
  }, { randomInt: deterministicRandomInt([0, 99]) });
});

test('파산채무는 보유 골드로 직접 상환할 수 있고 전액 상환 후 레버리지 진입이 풀린다', async () => {
  await withFixture(async ({ stocks, store }) => {
    await createJiminLongShortfallDebtScenario({ stocks, store, balance: 13_100_000 });
    ...
    assert.equal(repaid.requested, 13_000_000);
    assert.equal(repaid.repaid, JIMIN_LONG_LIQUIDATION_DEBT);
    assert.equal(repaid.bankruptcy.debt, 0);
    assert.equal(repaid.profile.balance, 98_000);
...
test('레버리지 명령 화면은 파산채무와 진입 제한을 안내한다', async () => {
  await withFixture(async ({ stocks, store }) => {
    await createJiminLongShortfallDebtScenario({ stocks, store });

    const portfolioInteraction = createStockInteraction('레버리지보유');
    await handleStockCommand(portfolioInteraction, stocks);

    assert.match(portfolioInteraction.replies[0], /파산채무: \*\*12,900,000골드\*\*/);
    assert.match(portfolioInteraction.replies[0], /새 레버리지 진입 불가/);

    const openInteraction = createStockInteraction('레버리지진입', {
      strings: { 종목: '도훈건설', 방향: '롱', 증거금: '1억' },
      integers: { 배율: 2 }
    });
    await handleStockCommand(openInteraction, stocks);

    assert.match(openInteraction.replies[0], /파산채무 12,900,000골드/);
...
test('채무상환 명령은 내 골드로 파산채무를 갚고 결과를 안내한다', async () => {
  await withFixture(async ({ stocks, store }) => {
    ...
    assert.match(repayInteraction.replies[0], /상환: \*\*12,900,000골드\*\*/);
...
test('레버리지 보유 화면은 처음 보는 만기 정산 손익을 상단에 요약한다', async () => {
  await withFixture(async ({ stocks, store }) => {
    await seedBalance(store, 'guild-1', 'user-1', '희희', 100_000);
    await stocks.openLeveragedPosition({
      guildId: 'guild-1',
    ...
    Date.now = () => 10 * STOCK_TICK_MS_FOR_TEST;
    try {
      const interaction = createStockInteraction('레버리지보유');
      await handleStockCommand(interaction, stocks);

      assert.match(interaction.replies[0], /이번 조회 정산 손익: \*\*\+10,380골드\*\*/);
      assert.match(interaction.replies[0], /1건 자동 정산/);
      assert.match(interaction.replies[0], /열려 있는 레버리지 거래가 없습니다/);
    } finally {
...
async function createJiminLongShortfallDebtScenario({
  stocks,
  store,
  guildId = 'guild-1',
  userId = 'user-1',
  ...
  await seedBalance(store, guildId, userId, username, balance);
  const opened = await stocks.openLeveragedPosition({
    guildId,
    userId,
    username,
  ...
  return { opened, portfolio };
}

async function forceMarketQuote(store, {
  guildId = 'guild-1',
  stockId,
  price,
  tickIndex,
  ...
  await store.update((data) => {
    const market = data.guilds?.[guildId]?.stocks?.market;
    const symbol = market?.symbols?.[stockId];

    assert.ok(symbol, `${stockId} market symbol should exist before forcing a quote`);
    const previousPrice = symbol.price;
    symbol.previousPrice = previousPrice;
    symbol.price = price;
    symbol.changeBps = Math.round((price - previousPrice) * 10_000 / previousPrice);
    symbol.news = `테스트 가격 고정: ${stockId} ${previousPrice}→${price}`;
    symbol.status = 'listed';
    symbol.eventType = symbol.changeBps < 0 ? 'crash' : 'surge';
    symbol.updatedAt = now;
    symbol.history = [
      ...(Array.isArray(symbol.history) ? symbol.history : []),
      { tickIndex, price, at: now }
    ];
    market.tickIndex = tickIndex;
