...
test('시장은 급등 이벤트를 quote 상태에 남기고 상장폐지 종목은 DB에서 삭제한다', async () => {
  ...
  await withFixture(async ({ stocks, store }) => {
    ...
    assert.equal(data.guilds['guild-1'].stocks.market.symbols.monkeynix, undefined);
    assert.match(data.guilds['guild-1'].stocks.delistedStocks.monkeynix.news, /상장폐지/);
    await assert.rejects(
...
test('상장폐지 종목은 보유, 주문, 알림, 레버리지에서 자동 정리된다', async () => {
  await withFixture(async ({ stocks, store }) => {
    ...
    assert.equal(dataAfterMarketCleanup.guilds['guild-1'].stocks.market.symbols.monkeynix, undefined);
    assert.equal(dataAfterMarketCleanup.guilds['guild-1'].stocks.users['user-1'].holdings.monkeynix, undefined);
    assert.equal(
      dataAfterMarketCleanup.guilds['guild-1'].stocks.users['user-1'].leveragedPositions[leveraged.position.id],
      undefined
    );
    ...
    assert.equal(data.guilds['guild-1'].stocks.market.symbols.monkeynix, undefined);
    assert.equal(data.guilds['guild-1'].stocks.users['user-1'].holdings.monkeynix, undefined);
  }, { randomInt: (min) => min });
});
