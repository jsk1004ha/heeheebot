...
test('레버리지 보유 화면은 id 복사 없이 버튼 닫기와 자동완성을 지원한다', async () => {
  await withFixture(async ({ stocks, store }) => {
    ...
    assert.equal(after.cash, 99_800);
  });
});
...
test('레버리지청산은 대상 생략과 번호 입력으로 쉽게 닫을 수 있다', async () => {
  await withFixture(async ({ stocks, store }) => {
    ...
    assert.match(closeOnlyOne.replies[0], /레버리지 청산 완료/);
  ...
  await withFixture(async ({ stocks, store }) => {
    ...
    assert.match(closeByNumber.replies[0], /레버리지 청산 완료/);
    const after = await stocks.getLeveragePortfolio({
      guildId: 'guild-1',
      userId: 'user-1',
      username: '희희',
...
test('파산채무는 보유 골드로 직접 상환할 수 있고 전액 상환 후 레버리지 진입이 풀린다', async () => {
  await withFixture(async ({ stocks, store }) => {
    ...
    await stocks.getLeveragePortfolio({
      guildId: 'guild-1',
      userId: 'user-1',
      username: '희희',
      now: 3 * 60 * 1000
