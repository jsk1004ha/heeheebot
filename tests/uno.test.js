...
test('우노 시작은 버튼형 방을 만들고 기본 규칙을 안내한다', async () => {
  ...
  assert.match(interaction.replies[0].content, /UNO ROOM/);
...
test('우노 로비 버튼은 참가자를 추가하고 방장이 게임을 시작한다', async () => {
  ...
  assert.match(start.channel.sends[0].content, /UNO GAME START/);
  assert.equal(start.channel.sends[0].components.length, 1);
  assert.equal(manager.getGame({ guildId: 'guild-1', channelId: 'channel-1' }).status, 'playing');
});

test('우노 내 손패 버튼은 참가자에게만 비공개 손패를 보여준다', async () => {
  const manager = new UnoGameManager({ now: () => 3_000, randomInt: () => 0 });
  await handleUnoCommand(createCommandInteraction({ subcommand: '시작' }), manager, quietLogger);
  const lobby = manager.getGame({ guildId: 'guild-1', channelId: 'channel-1' });
  await handleUnoCommand(createButtonInteraction({
    customId: `uno_join:${lobby.id}`,
    user: { id: 'user-2', username: '둘째' }
  }), manager, quietLogger);
  await handleUnoCommand(createButtonInteraction({
  ...
  const started = manager.getGame({ guildId: 'guild-1', channelId: 'channel-1' });

  const hand = createButtonInteraction({
    customId: `uno_hand:${started.id}`,
    user: { id: 'user-2', username: '둘째' }
  });
  await handleUnoCommand(hand, manager, quietLogger);

  assert.equal(hand.replies.length, 1);
  assert.equal(hand.replies[0].flags, MessageFlags.Ephemeral);
  assert.match(hand.replies[0].content, /내 우노 손패/);
  assert.match(hand.replies[0].content, /카드 선택 번호/);
