...
test('희진 다마고치 명령 payload는 상태, 스킨, 꾸미기, 에셋 명령을 등록한다', () => {
...
test('희진 에셋 명령은 미리보기 PNG를 디스코드 embed 이미지로 보낸다', async () => {
  const fixture = createFixture();
  ...
  try {
    ...
    assert.ok(payload.files.length >= 3);
    assert.ok(payload.embeds.length >= 3);
    assert.ok(payload.embeds.every((embed) => embed.data.image?.url?.startsWith('attachment://')));
    const embedImageNames = new Set(payload.embeds.map((embed) => embed.data.image.url.replace('attachment://', '')));
    assert.ok(payload.files.every((file) => embedImageNames.has(file.name)));
    assert.ok(payload.embeds.some((embed) => embed.data.title.includes('스킨 미리보기')));
    assert.ok(payload.embeds.some((embed) => embed.data.title.includes('꾸미기/도구')));
  } finally {
    fixture.cleanup();
  }
});
