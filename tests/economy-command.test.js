...
test('경제 명령 payload는 경험치를 단일 표현으로 설명한다', () => {
  ...
  assert.deepEqual(loanCommand.options.map((option) => option.name), ['대상', '돈', '이자', '기간', '이자주기', '이자방식']);
  assert.equal(loanCommand.options.find((option) => option.name === '대상').required, true);
  assert.equal(loanCommand.options.find((option) => option.name === '돈').required, true);
  ...
  assert.deepEqual(lendCommand.options.map((option) => option.name), ['대상']);
...
test('돈빌리기 요청은 빌려줄 유저에게 간단 승인 안내 DM을 보낸다', async () => {
  ...
  assert.match(sent[0].content, /\/돈빌려주기 대상:테스터/);
  assert.doesNotMatch(sent[0].content, /행동:수락|이자방식|이자기간/);
