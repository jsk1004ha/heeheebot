...
test('돈빌리기는 대상과 돈만으로 기본 기간과 상환방식을 채운다', async () => {
  const calls = [];
  const economy = {
  ...
  assert.match(interaction.replies[0].content, /수락/);
});
...
test('돈빌리기 수락과 결정은 복잡한 조건을 생략하면 기본값으로 처리한다', async () => {
  const calls = [];
  const economy = {
    async offerUserLoan(payload) {
      calls.push(['offer', payload]);
  ...
  await handleEconomyCommand(createInteraction('돈빌리기', {
    targetUser,
    stringOptions: { 행동: 'offer' }
  }), economy);
  await handleEconomyCommand(createInteraction('돈빌리기', {
    targetUser,
    stringOptions: { 행동: 'decide' }
  }), economy);

  assert.equal(calls[0][1].interestPercent, 0);
  assert.equal(calls[0][1].interestPeriodHours, 24);
  assert.equal(calls[0][1].interestType, 'simple');
  assert.equal(calls[1][1].accept, true);
});
