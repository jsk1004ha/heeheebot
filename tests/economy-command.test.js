...
test('경제 명령 payload는 경험치를 단일 표현으로 설명한다', () => {
  ...
  const transferCommand = payloads.find((command) => command.name === '송금');
  const loanCommand = payloads.find((command) => command.name === '돈빌리기');
  const lendCommand = payloads.find((command) => command.name === '돈빌려주기');
  const repayCommand = payloads.find((command) => command.name === '돈갚기');
  ...
  assert.ok(transferCommand);
  assert.ok(loanCommand);
  assert.ok(lendCommand);
  assert.ok(repayCommand);
  assert.equal(payloads.find((command) => command.name === '돈빌리기수락'), undefined);
  assert.equal(payloads.find((command) => command.name === '돈빌리기현황'), undefined);
  ...
  assert.equal(transferCommand.options.find((option) => option.name === '금액').type, 3);
  assert.deepEqual(loanCommand.options.map((option) => option.name), ['행동', '대상', '돈', '선택', '이자', '기간', '이자주기', '이자방식']);
  assert.deepEqual(
    loanCommand.options.find((option) => option.name === '행동').choices.map((choice) => choice.value),
    ['request', 'status', 'decide']
  );
  assert.equal(Boolean(loanCommand.options.find((option) => option.name === '대상').required), false);
  assert.equal(Boolean(loanCommand.options.find((option) => option.name === '돈').required), false);
  assert.equal(loanCommand.options.find((option) => option.name === '돈').type, 3);
  assert.deepEqual(
    loanCommand.options.find((option) => option.name === '선택').choices.map((choice) => choice.value),
    ['accept', 'reject']
  );
  assert.deepEqual(
    loanCommand.options.find((option) => option.name === '이자').choices.map((choice) => choice.value),
    [3, 5, 10, 15, 20]
  );
  assert.deepEqual(
    loanCommand.options.find((option) => option.name === '기간').choices.map((choice) => choice.value),
    [24, 48, 72]
  );
  assert.deepEqual(
    loanCommand.options.find((option) => option.name === '이자주기').choices.map((choice) => choice.value),
    [1, 2, 3]
  );
  assert.deepEqual(
    loanCommand.options.find((option) => option.name === '이자방식').choices.map((choice) => choice.value),
    ['simple', 'compound']
  );
  assert.deepEqual(
    lendCommand.options.map((option) => option.name),
    ['대상', '행동', '돈', '이자', '기간', '이자주기', '이자방식']
  );
  assert.equal(lendCommand.options[0].required, true);
  assert.deepEqual(
    lendCommand.options.find((option) => option.name === '행동').choices.map((choice) => choice.value),
    ['accept', 'reject', 'offer', 'propose']
  );
  assert.equal(lendCommand.options.find((option) => option.name === '돈').type, 3);
  assert.ok(repayCommand.options.some((option) => option.name === '대상' && option.required));
  assert.ok(repayCommand.options.some((option) => option.name === '돈' && !option.required));
  assert.equal(repayCommand.options.find((option) => option.name === '돈').type, 3);
...
test('돈빌리기는 대상과 돈만으로 기본 복리 조건을 채운다', async () => {
  const calls = [];
  const economy = {
    async requestUserLoan(payload) {
      ...
      return {
        request: {
          ...
          interestBps: payload.interestPercent * 100,
  ...
  const interaction = createInteraction('돈빌리기', {
    targetUser,
    integerOptions: { 돈: 500 }
  });

  await handleEconomyCommand(interaction, economy);

  assert.equal(calls[0].termHours, 24);
  assert.equal(calls[0].interestPercent, 5);
  assert.equal(calls[0].interestPeriodHours, 3);
  assert.equal(calls[0].interestType, 'compound');
  assert.equal(calls[0].repaymentMode, 'installment');
  assert.match(interaction.replies[0].content, /24시간 0분/);
  assert.match(interaction.replies[0].content, /5% 복리/);
  assert.match(interaction.replies[0].content, /이자 주기: \*\*3시간 0분\*\*/);
  assert.match(interaction.replies[0].content, /돈빌려주기/);
});
...
test('송금 응답은 대상 유저를 원시 객체가 아니라 실제 멘션으로 표시한다', async () => {
  ...
  assert.equal(calls[0].amount, 100_000_000);
...
test('돈빌리기 명령은 선택한 이자와 기간을 대출 요청으로 전달한다', async () => {
  const calls = [];
  const economy = {
    async requestUserLoan(payload) {
      calls.push(payload);
  ...
  const interaction = createInteraction('돈빌리기', {
    targetUser,
    integerOptions: { 기간: 48, 이자: 15, 이자주기: 2 },
    stringOptions: { 돈: '1조', 이자방식: 'simple' }
  });
  ...
  assert.match(interaction.replies[0].content, /<@user-2>/);
  assert.match(interaction.replies[0].content, /15% 단리/);
  assert.match(interaction.replies[0].content, /48시간 0분/);
  assert.match(interaction.replies[0].content, /이자 주기: \*\*2시간 0분\*\*/);
});

test('돈빌리기 요청은 빌려줄 유저에게 간단 승인 안내 DM을 보낸다', async () => {
  const sent = [];
  const economy = {
    async requestUserLoan(payload) {
      return {
  ...
  assert.match(sent[0].content, /5% 복리/);
  assert.match(sent[0].content, /\/돈빌려주기 대상:테스터 행동:수락/);
  assert.match(sent[0].content, /\/돈빌려주기 대상:테스터 행동:거절/);
  assert.match(sent[0].content, /\/돈빌려주기 대상:테스터 행동:조건제시/);
  assert.match(sent[0].content, /\/돈빌리기 행동:현황/);
  assert.doesNotMatch(sent[0].content, /이자방식|이자기간/);
...
test('돈빌려주기는 상대 요청을 한 번에 승인하고 돈을 이동한다', async () => {
  const calls = [];
  const economy = {
    async acceptUserLoanRequest(payload) {
      calls.push(payload);
      return {
        accepted: true,
        loan: {
          ...
          totalDue: 1_000,
          interestBps: 500,
          interestPeriodMs: 3 * 60 * 60 * 1000,
          interestType: 'compound',
          dueAt: 24 * 60 * 60 * 1000,
          repaymentMode: 'installment'
        },
        borrower: { balance: 1_000 },
        lender: { balance: 999_000 }
      };
    }
  };
  const targetUser = {
  ...
  const interaction = createInteraction('돈빌려주기', {
    targetUser
  });
  ...
  assert.match(interaction.replies[0].content, /<@user-2>/);
  assert.match(interaction.replies[0].content, /1,000골드/);
  assert.match(interaction.replies[0].content, /5% 복리/);
  assert.match(interaction.replies[0].content, /돈갚기/);
});

test('돈빌리기 행동:현황은 내 요청, 받은 요청, 수락/거절 안내를 한 화면에 보여준다', async () => {
  const calls = [];
  const economy = {
    async getUserLoanStatus(payload) {
      ...
      return {
        ...
        outgoingRequests: [
          {
            status: 'offered',
            lenderUserId: 'user-2',
            lenderUsername: '빌려줄사람',
            amount: 1_000,
            totalDue: 1_100,
            termMs: 24 * 60 * 60 * 1000,
            interestBps: 500,
            interestPeriodMs: 3 * 60 * 60 * 1000,
            interestType: 'compound',
            repaymentMode: 'installment'
        ...
        incomingRequests: [
          {
            status: 'requested',
            borrowerUserId: 'user-3',
            borrowerUsername: '빌리고싶은사람',
            amount: 2_000,
            totalDue: 2_000,
            termMs: 48 * 60 * 60 * 1000,
            interestBps: 0,
            interestPeriodMs: 3 * 60 * 60 * 1000,
            interestType: 'simple',
            repaymentMode: 'installment'
  ...
  assert.match(interaction.replies[0].content, /내 결정 필요/);
  assert.match(interaction.replies[0].content, /내 수락·거절·조건제시 필요/);
  assert.match(interaction.replies[0].content, /\/돈빌리기 행동:결정 대상:빌려줄사람 선택:수락/);
  assert.match(interaction.replies[0].content, /\/돈빌리기 행동:결정 대상:빌려줄사람 선택:거절/);
  assert.match(interaction.replies[0].content, /\/돈빌려주기 대상:빌리고싶은사람 행동:수락/);
  assert.match(interaction.replies[0].content, /\/돈빌려주기 대상:빌리고싶은사람 행동:거절/);
  assert.match(interaction.replies[0].content, /\/돈빌려주기 대상:빌리고싶은사람 행동:조건제시/);
});

test('돈빌려주기는 나에게 온 요청을 거절할 수 있다', async () => {
  const calls = [];
  const economy = {
    async rejectUserLoanRequest(payload) {
      calls.push(payload);
      return {
        rejected: true,
        request: {
  ...
  assert.match(interaction.replies[0].content, /거절/);
  assert.match(interaction.replies[0].content, /5,000골드/);
});

test('돈빌려주기는 받은 요청에 이자 조건을 제시할 수 있다', async () => {
  const calls = [];
  const economy = {
  ...
  assert.match(interaction.replies[0].content, /조건을 제시/);
  assert.match(interaction.replies[0].content, /15% 단리/);
});

test('돈빌려주기는 먼저 돈과 조건을 제시해 상대가 결정하게 할 수 있다', async () => {
  const calls = [];
  const economy = {
    async createUserLoanOffer(payload) {
      calls.push(payload);
      return {
        offer: {
          amount: payload.amount,
          totalDue: payload.amount,
          interestBps: payload.interestPercent * 100,
          interestPeriodMs: payload.interestPeriodHours * 60 * 60 * 1000,
          interestType: payload.interestType,
          termMs: payload.termHours * 60 * 60 * 1000,
  ...
  const interaction = createInteraction('돈빌려주기', {
    targetUser,
    integerOptions: { 기간: 48, 이자: 10, 이자주기: 2 },
    stringOptions: { 행동: 'propose', 돈: '1억', 이자방식: 'compound' }
  });
  ...
  assert.match(interaction.replies[0].content, /대출 조건을 먼저 제시/);
  assert.match(interaction.replies[0].content, /100,000,000골드/);
  assert.match(interaction.replies[0].content, /\/돈빌리기 행동:결정 대상:테스터 선택:수락/);
});

test('돈빌리기 행동:결정은 제시받은 조건을 수락하거나 거절한다', async () => {
  const calls = [];
  const economy = {
  ...
  const interaction = createInteraction('돈빌리기', {
    targetUser,
    stringOptions: { 행동: 'decide', 선택: 'reject' }
  });
...
test('돈갚기는 별도 명령으로 금액을 비우면 가능한 만큼 상환한다', async () => {
  const calls = [];
  const economy = {
    async repayUserLoan(payload) {
      calls.push(payload);
      return {
        repaid: 300,
        remaining: 700,
  ...
  const interaction = createInteraction('돈갚기', {
    targetUser
  });

  await handleEconomyCommand(interaction, economy);

  assert.equal(calls[0].amount, null);
  assert.match(interaction.replies[0].content, /300골드/);
  assert.match(interaction.replies[0].content, /남은 대출금/);
});

test('돈갚기는 한국식 큰 단위 금액도 상환 요청으로 전달한다', async () => {
  const calls = [];
  const economy = {
    async repayUserLoan(payload) {
      calls.push(payload);
      return {
        repaid: payload.amount,
        remaining: 0,
        borrower: { balance: 0 }
      };
    }
  ...
  assert.equal(calls[0].amount, '10000000000000000');
  assert.match(interaction.replies[0].content, /10,000,000,000,000,000골드/);
});
