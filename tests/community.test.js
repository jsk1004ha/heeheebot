...
test('커뮤니티 명령 payload는 업적, 칭호, 미션, 복권, 상점, 서버이벤트를 등록한다', () => {
  ...
  assert.equal(lotteryBuyQuantity.max_value, undefined);
  assert.equal(lotteryBulkQuantity.max_value, undefined);
...
test('복권은 보유 골드 한도 내에서 100장 초과 구매를 허용한다', async () => {
  const fixture = await createFixture({ randomInt: () => 1 });
  ...
  try {
    ...
    assert.equal(buy.entries.length, 150);
...
test('커뮤니티 명령 핸들러는 복권 구매와 미션 응답을 반환한다', async () => {
  ...
  try {
    ...
    assert.match(lotteryStatusInteraction.replied.content, /구매 제한: 보유 골드 한도 내 제한 없음/);
