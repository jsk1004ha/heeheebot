...
test('오늘할일 미션 버튼은 수령한 커뮤니티 미션에 시즌 포인트를 붙인다', async () => {
  const interaction = createTodayButtonInteraction('today_missions:user-1');
  const services = createServices({
    profile: { lastDailyDay: 20_000, lastFortuneXpDay: null, dailyStreak: 1 },
    dailyMissions: [],
    rpgDailyMissions: [],
    swordStatus: { giftAvailable: false, giftRemainingMs: 60_000, battleRemaining: 10, battleStoneRemaining: 5 },
    claimMissionsResult: {
      claimed: [
        mission('daily_checkin', '출석 찍기', true, false, '완료'),
        mission('daily_lottery', '복권 한 장', true, false, '완료'),
        mission('daily_chat', '채팅하기', true, false, '완료')
      ],
      totalCoins: 300,
      totalXp: 60
    },
    seasonOverview: {
      ...
      daily: { earned: 0, cap: 300, remaining: 300 },
      rewards: []
    },
    seasonChallenges: { daily: [], weekly: [] }
  });

  const handled = await handleTodayCommand(interaction, services);

  assert.equal(handled, true);
  assert.equal(services.community.claimedMissions, true);
  assert.deepEqual(services.seasons.awards.map(({ source, points }) => ({ source, points })), [
    { source: 'community_mission_claim', points: 40 }
  ]);
  assert.equal(interaction.updates.length, 1);
  assert.match(interaction.updates[0].content, /커뮤니티 일일 미션 보상 수령/);
  assert.match(interaction.updates[0].content, /시즌: 테스트 시즌/);
});
