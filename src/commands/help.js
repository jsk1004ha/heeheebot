...
const HELP_CATEGORIES = Object.freeze({
  home: Object.freeze({
    ...
    fields: Object.freeze([
      ...
      { name: '경제/놀이', value: '`/주식 시세` `/광산` `/광산 행동:광석 판매` `/재생` `/검색` `/낚시` `/카지노정보` `/희진다마고치`' }
    ])
  }),
  ...
  music: Object.freeze({
    ...
    fields: Object.freeze([
      ...
      { name: '서버 기록/진단', value: '`/랭킹 종류:인기곡` `/내음악통계` `/노드상태`로 인기곡, 개인 신청 통계, Lavalink 상태를 확인합니다.' }
    ])
  }),
  games: Object.freeze({
    label: '놀이',
    ...
    fields: Object.freeze([
      ...
      { name: '광산', value: '`/광산` 하나에서 `행동:도감/곡괭이 강화/채굴 잠수/광석 판매/광석 시세`를 선택합니다. 버튼으로 연속 채굴/판매 가능' },
      ...
      { name: "Liar's Bar", value: '`/라이어바 시작` `/라이어바 손패` `/라이어바 내기` `/라이어바 라이어` — 2~4명이 카드 주장을 의심하고 생존하는 라이어스 바식 게임' },
      { name: '마피아게임', value: '`/마피아게임 시작` `/마피아게임 상태` — 밤 행동과 과반 투표로 진행하는 추리 게임' },
