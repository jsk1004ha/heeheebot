...
function createTimingHardTargetPrompt(targetSeconds, options = {}) {
  ...
  const prompts = [
    ...
    `(${target + 10}초 × 3 - 30초) ÷ 3`,
    `방정식 2x + ${target - 4} = ${3 * target - 4}일 때 x초`,
    `등차수열 첫째항 ${target - 2}, 공차 1의 셋째항초`,
    `공정한 주사위에서 짝수 눈의 개수${formatTimingSignedSeconds(target - 3)}`,
    `공정한 동전 2번에서 앞면이 적어도 한 번 나오는 경우의 수${formatTimingSignedSeconds(target - 3)}`,
    `${target + 5}C1 - 5초`,
    `삼각수 T${target + 1} - T${target} - 1초`,
    `|-${target}|초`,
    `최대공약수(${target * 2}, ${target * 3})초`,
    `log₂ ${2 ** target}초`,
    `한 변 ${target + 2}인 정사각형 둘레 ÷ 4 - 2초`,
    `${target + 30}초 - 0.5분`,
    `1부터 ${2 * target - 1}까지 홀수의 개수초`,
    `${target - 2}와 ${target + 2}의 평균초`,
    `(${target + 1})² - ${target ** 2 + target + 1}초`,
    `3²${formatTimingSignedSeconds(target - 9)}`,
    `5! ÷ 12${formatTimingSignedSeconds(target - 10)}`,
    `2×${target + 4}초 - ${target + 8}초`,
    `(${target * 6}초 ÷ 3) - ${target}초`,
    `방정식 x + ${target + 4} = ${2 * target + 4}일 때 x초`,
    `10²의 제곱근${formatTimingSignedSeconds(target - 10)}`,
    `1분 ÷ 3${formatTimingSignedSeconds(target - 20)}`,
    `정삼각형 한 변 ${target + 3}의 둘레 ÷ 3 - 3초`,
    `5명 중 대표 1명 뽑는 경우의 수${formatTimingSignedSeconds(target - 5)}`,
    `2³ + 2²${formatTimingSignedSeconds(target - 12)}`
  ];
...
function createTimingVeryHardTargetPrompt(targetSeconds, options = {}) {
  ...
  const prompts = [
    ...
    `2×2 행렬 [${target + 1}, 1; 2, 2]의 행렬식 ÷ 2초`,
    `합동식 7x ≡ 1 (mod 20)의 1~20 자연수 해${formatTimingSignedSeconds(target - 3)}`,
    `${target + 1}번째 삼각수 - ${target}번째 삼각수 - 1초`,
    `P(A∩B)=3/10, P(B)=1/2일 때 조건부확률 P(A|B)×10${formatTimingSignedSeconds(target - 6)}`,
    `연립방정식 x+y=${target + 7}, x-y=${target - 7}에서 x의 값`,
    `정적분 ∫₀² ${target}x dx ÷ 2초`,
    `함수 f(x)=${target}x²에서 x=1일 때 순간변화율 ÷ 2초`,
    `복소수 z=3+4i일 때 |z|² - 15${formatTimingSignedSeconds(target - 10)}`,
    `log₂ 1024${formatTimingSignedSeconds(target - 10)}`,
    `1부터 30까지 자연수 중 2 또는 3의 배수 개수${formatTimingSignedSeconds(target - 20)}`,
    `Σₖ₌₁⁴ k² ÷ 3${formatTimingSignedSeconds(target - 10)}`,
    `서로 다른 6명을 원형으로 앉히는 경우의 수 ÷ 24${formatTimingSignedSeconds(target - 5)}`,
    `오일러 피 함수 φ(20)${formatTimingSignedSeconds(target - 8)}`,
    `4×4 격자에서 최단경로 수 ÷ 5${formatTimingSignedSeconds(target - 14)}`,
    `다항식 (1+x)^6의 x³ 계수 ÷ 2${formatTimingSignedSeconds(target - 10)}`,
    `3자리 이진 문자열 중 1이 적어도 하나 있는 경우의 수${formatTimingSignedSeconds(target - 7)}`,
    `1,2,3,4,5에서 서로 다른 세 수를 골라 합이 9가 되는 경우의 수 + ${target - 2}초`,
    `평균이 12인 다섯 수의 총합 ÷ 6${formatTimingSignedSeconds(target - 10)}`,
    `다항식 x²-${2 * target}x+${target ** 2}의 중근`,
    `수열 aₙ=2ⁿ에서 a₅ ÷ 4${formatTimingSignedSeconds(target - 8)}`,
    `서로 다른 5명 중 회장·부회장을 순서 있게 뽑는 경우의 수 ÷ 2${formatTimingSignedSeconds(target - 10)}`,
    `7명의 학생을 A반 3명, B반 4명으로 나누는 경우의 수 ÷ 5${formatTimingSignedSeconds(target - 7)}`,
    `정육각형의 대각선 개수${formatTimingSignedSeconds(target - 9)}`,
    `1부터 100까지 자연수 중 제곱수의 개수${formatTimingSignedSeconds(target - 10)}`,
    `x²≡1 (mod 8)을 만족하는 1~8 자연수 x의 개수${formatTimingSignedSeconds(target - 4)}`,
    `서로 다른 주사위 3개 눈의 합이 5가 되는 경우의 수${formatTimingSignedSeconds(target - 6)}`,
    `함수 f(x)=x³-${target}x에서 x=0일 때 접선 기울기의 절댓값`
  ];
